...
app.put("/api/private/notes/:id", (req, res) => {
  try {
    ...
    if (index !== -1) {
      ...
      return res.json({
        success: true,
        message: "Đã sửa ghi chú riêng tư thành công",
      });
    }
    res.status(404).json({ message: "Không tìm thấy ghi chú" });
  } catch (error) {
    res.status(500).json({ message: "Lỗi sửa ghi chú riêng tư" });
  }
});

// 5. Xóa ghi chú riêng tư (DELETE)
app.delete("/api/private/notes/:id", (req, res) => {
  try {
    ...
    res.json({ success: true, message: "Đã xóa ghi chú riêng tư thành công" });
  } catch (error) {
